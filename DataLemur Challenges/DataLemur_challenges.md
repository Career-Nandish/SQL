...
## [40. [Hard] Advertiser Status](https://datalemur.com/questions/updated-status)
...
SELECT COALESCE(a.user_id, d.user_id) AS user_id,
      CASE
          ...
          WHEN d.user_id IS NOT NULL AND 
               a.status IN ('NEW', 'EXISTING', 'RESURRECT') THEN 'EXISTING'
          WHEN d.user_id IS NOT NULL AND 
               a.status = 'CHURN' THEN 'RESURRECT'
...
## [41. [Hard] 3-Topping Pizzas](https://datalemur.com/questions/pizzas-topping-cost)
...
```SQL
SELECT CONCAT_WS(',', t1, t2, t3) AS pizza,
       c1+c2+c3 AS total_cost
FROM (
    SELECT p1.topping_name AS t1, p1.ingredient_cost AS c1,
           p2.topping_name AS t2, p2.ingredient_cost AS c2,
           p3.topping_name AS t3, p3.ingredient_cost AS c3
